...
function Task({ task, onToggleStatus, onEdit, onDelete, onEditChange, onEditSubmit, isEditing, editingTaskTitle ,openCalendar,
                  setOpenCalendarId,onDateSelect,selectedDate
              }) {
    return (
        <li key={task.id} className={`task ${task.status ? 'completed' : ''}`}>
            <input type="checkbox" checked={task.status} onChange={onToggleStatus} />
            {isEditing ? (
                <>
                    <Input
                        type="text"
                ...
                <>
                    ...
                    <MyButton
                        name={<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
